...
impl AndroidAudioPlayer {
    ...
    fn download_with_prebuffer(&self, url: &str, dest_path: &str) -> Result<()> {
        use std::io::Write;
        use std::fs::File;
...
impl AudioPlayer for AndroidAudioPlayer {
    ...
    fn load_url(&mut self, url: &str) -> Result<()> {
        ...
        *self.sample_count.lock() = 0;
        ...
        // Download audio file to temporary location with progressive loading
        let temp_file_path = format!("/tmp/rust_audio_stream_{}.tmp",
        ...
        match self.download_with_prebuffer(url, &temp_file_path) {
            Ok(()) => {
