...
build_android() {
    ...
    # Format: target_triple:architecture:android_abi
    ...
    for target_config in "${ANDROID_TARGETS[@]}"; do
        ...
        cp "target/$TARGET/release/librust_rss_parser.so" "$ABI_OUTPUT_DIR/"
    ...
    for abi_dir in "$ANDROID_OUTPUT_DIR"/*; do
        if [ -d "$abi_dir" ]; then
            ...
            cp "$abi_dir"/librust_rss_parser.so "$JNILIBS_DIR/$ABI_NAME/"
        fi
    done
...
build_macos() {
    ...
    cp "target/x86_64-apple-darwin/release/librust_rss_parser.dylib" "$MACOS_OUTPUT_DIR/" || \
    cp "target/x86_64-apple-darwin/release/librust_rss_parser.a" "$MACOS_OUTPUT_DIR/" || true
    ...
    cp "target/aarch64-apple-darwin/release/librust_rss_parser.dylib" "$MACOS_ARM_OUTPUT_DIR/" || \
    cp "target/aarch64-apple-darwin/release/librust_rss_parser.a" "$MACOS_ARM_OUTPUT_DIR/" || true
    ...
    if [ -f "target/x86_64-apple-darwin/release/librust_rss_parser.dylib" ] && \
       [ -f "target/aarch64-apple-darwin/release/librust_rss_parser.dylib" ]; then
        lipo -create \
...
# Main build process
main() {
    print_info "Starting cross-platform build for rust-rss-parser..."
